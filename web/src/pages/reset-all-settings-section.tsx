...
export function ResetAllSettingsSection({
  ...
  return (
    <>
      {/* FLAT như mọi mục khác - đây là một hành động phụ ở cuối nhóm, không
          phải cảnh báo cần giật mắt người dùng bằng cả một khối màu */}
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2 text-[14px] font-semibold text-ink">
            <IconWarning size={15} className="text-amber-500" />
          ...
          {xong && <p className="mt-2 text-[12px] text-emerald-600">{xong}</p>}
        </div>
        <button
          type="button"
          onClick={() => void datLai()}
          disabled={dangChay || soODaDoi === 0}
          className="shrink-0 rounded-lg border border-line bg-white px-3 py-1.5 text-[13px] font-semibold text-ink-soft hover:bg-tile disabled:opacity-50"
        >
