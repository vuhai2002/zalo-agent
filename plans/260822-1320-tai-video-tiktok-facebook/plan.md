...
| **Ưu tiên `sendVideo({videoUrl})`** | 0 byte video qua VPS |
| **Vẫn phải có nhánh tải-rồi-upload** | Chưa kiểm chứng được Zalo có nhận URL ngoài không |
| **Chọn format theo codec, không theo ID** | Mã format đổi âm thầm; `vcodec^=avc` thì không |
| **Song song 2, còn lại xếp hàng** | ~75 MB RAM mỗi tiến trình yt-dlp |
